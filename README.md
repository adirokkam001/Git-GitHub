...
> A practical, example-driven guide to Git and GitHub for DevOps engineers — from core concepts to real-world workflows and CI/CD basics.
...
4. [Staging Area](#4-staging-area)
5. [Commit](#5-commit)
6. [The Most Important Git Workflow](#6-the-most-important-git-workflow)
7. [Branch](#7-branch)
8. [Merge](#8-merge)
9. [Remote Repository](#9-remote-repository)
10. [HEAD](#10-head)
11. [.gitignore](#11-gitignore)
12. [Git Commands](#git-commands)
    - [git init](#12-git-init) · [git clone](#13-git-clone) · [git status](#14-git-status) · [git add](#15-git-add) · [git commit](#16-git-commit) · [git push](#17-git-push) · [git pull](#18-git-pull) · [git fetch](#19-git-fetch) · [git branch](#20-git-branch) · [git checkout](#21-git-checkout) · [git switch](#22-git-switch) · [git merge](#23-git-merge) · [git log](#24-git-log) · [git diff](#25-git-diff) · [git stash](#26-git-stash) · [git reset](#27-git-reset) · [git revert](#28-git-revert)
13. [GitHub](#github)
    - [Create a Repository](#29-create-a-github-repository) · [SSH Authentication](#30-ssh-authentication) · [Personal Access Token](#31-personal-access-token--pat) · [Pull Request](#32-pull-request--pr) · [Branch Protection](#33-branch-protection) · [README](#34-readme) · [GitHub Actions](#35-github-actions-basics)
14. [Complete Workflow Examples](#-complete-git-workflow-example)
15. [Git Architecture to Memorize](#-the-git-architecture-you-should-memorize)
16. [Commands Cheat Sheet](#-commands-cheat-sheet)

---

Git is one of the most important tools in DevOps — engineers constantly work with source code, configuration files, infrastructure code, CI/CD pipelines, and deployment files.
...
### Git
**Git = Version Control System.** Software installed on your computer that tracks changes in your files.

**Example** — `index.html` changes from `Hello World` to `Hello DevOps`. Git remembers:
...
### GitHub
**GitHub = Cloud platform for hosting Git repositories.** Git works locally; GitHub stores your repository online and enables collaboration.

```
Git → Your computer → Tracks changes
GitHub → Internet/Cloud → Stores your Git repository → Collaboration
```

**Simple analogy:** Git is like Microsoft Word's version history. GitHub is like Google Drive, where you store and share the project.
...
When Git tracks this project, it becomes a **Git repository**, containing a hidden directory: `.git/` — Git's internal history data.
...
When you open and modify `index.html`, that change first exists in your working directory.
...
**Example** — you modify `index.html`, `style.css`, and `README.md`, but only want to commit the first two:
...
After finishing your feature, you combine it with `main` — this is a **merge**.
...
A **remote repository** is a Git repository located somewhere else — usually GitHub.

```
Your Mac → Local Repository → git push → GitHub (Remote Repository)
```
...
**HEAD** = the commit/branch you are currently pointing to.
...
If you're on `main` at commit C: `HEAD → main → C`. A new commit shifts HEAD forward:
...
Understanding HEAD matters most when learning `git reset`, `git checkout`, `git switch`, and `git revert`.
...
**Why?** You usually don't want to upload passwords, API keys, secret files, temporary files, large generated files, or dependencies.

**Example** — `.env` might contain:
```
DB_PASSWORD=secret123
AWS_ACCESS_KEY=xxxxx
```

> ⚠️ You should **never** commit secrets to GitHub.
...
Git creates `my-project/.git/` and starts tracking the project. Check with:
...
Use `clone` when a project already exists on GitHub.
...
Shows modified files, untracked files, staged files, and current branch. Use this command **very frequently**.
...
| Good | Bad |
|------|-----|
| `git commit -m "Add EC2 deployment script"` | `git commit -m "changes"` |
...
**Example:** Your friend changes `README.md` and pushes it to GitHub. You run `git pull` to receive the changes.

---

### 19. `git fetch`

Downloads information about changes from the remote repository **without merging** them into your current branch.

```bash
git fetch
```

You can then inspect and decide what to do with the changes.
...
The `*` marks your current branch. Create a branch:
...
Modern Git command specifically for branch switching.
...
> 💡 Recommended: learn the modern `git switch` syntax for branch operations.
...
> ⚠️ **Direction matters.** If you're on `main` and run `git merge feature-login`, you're bringing `feature-login` **into** `main`.
...
Think of it as: *"What did I modify?"*
...
**Scenario:** You're working on `feature-login` with unfinished work, but there's a production bug to fix immediately.

```bash
git stash              # Save uncommitted work, clean working directory
git switch main        # Switch and fix the bug
git switch feature-login
git stash pop          # Restore your changes
```

```
Uncommitted work → git stash → Temporarily stored → Do other work → git stash pop → Changes restored
```
...
Moves HEAD and can change what is staged or what is in your working directory, depending on the mode.

**Unstage a file** (keep file changes):
```bash
...
> ⚠️ Be very careful with `git reset --hard` — it can **permanently discard uncommitted work**.
...
```
A → B → C → D   (D reverses the changes made by C)
```

This is generally **safer for shared branches** like `main`, since it doesn't rewrite history.
...
| Command | Effect |
|---------|--------|
| `git reset`  | Moves branch history backward (`A → B → C` becomes `A → B`) |
| `git revert` | Creates a new commit that undoes previous changes (`A → B → C` becomes `A → B → C → D`) |

For shared/public history, **revert is usually preferred** since it doesn't rewrite existing commit history.
...
A good way to demonstrate your DevOps learning to recruiters.

---

### 30. SSH Authentication

SSH lets your computer authenticate with GitHub using a key pair instead of repeatedly entering credentials.
...
A **Personal Access Token** is a credential used for GitHub authentication when a token is required instead of a password.

GitHub commonly uses **Username + Personal Access Token**, or preferably an **SSH key**, for Git operations.

> ⚠️ Never publish your PAT in GitHub, a README, a Git repository, or screenshots. Treat it like a password.
...
**Developer flow:**
```
Create feature branch → Write code → git add → git commit → git push → Create Pull Request
```

Another developer can then review code, comment, request changes, or approve.
...
**Branch protection rules** help prevent dangerous changes to important branches (e.g., `main`), typically requiring:

```
Pull Request + Code Review + Status Checks → before merging
```

**Typical professional workflow:**
```
Developer → Feature branch → Pull Request → Code Review → CI tests → Approval → main
```
...
GitHub automatically displays the README on your repository's main page — good READMEs show recruiters what you learned **and** what you actually practiced.
...
```
Developer → git push → GitHub → GitHub Actions → Test → Build → Deploy
```

A GitHub Actions workflow is stored at:
...
Later in your DevOps journey, this connects to:
...
# Step 3 — Create files (index.html, style.css)
...
# Step 7 — Create a GitHub repository named "my-website"

# Step 8 — Connect local repo to GitHub (SSH)
...
```
Your Mac → Local Git Repository → git push → GitHub
```
...
# Work on login.html, then check status
git status
...
```
feature-login → Pull Request → Code Review → Approval → Merge → main
```
...
*📌 Notes compiled for personal study — Git & GitHub fundamentals for DevOps.*
itHub ⭐⭐⭐
